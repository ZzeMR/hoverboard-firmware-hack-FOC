...
static uint16_t wp_l_previous = 0;
static uint16_t wp_r_previous = 0;

int16_t modulo(int16_t m, int16_t rest_classes){
  return (((m % rest_classes) + rest_classes) %rest_classes);
}

int16_t up_or_down(int16_t previous, int16_t afterwards){
  uint16_t up_down[6] = {0,-1,-2,0,2,1};
  //uint16_t mod_diff =  (((previous - nachher) % 6) + 6) % 6;

  return up_down[modulo(previous-afterwards, 6)];
}
...
void DMA1_Channel1_IRQHandler(void) {
  ...
  // motAngleLeft = rtY_Left.a_elecAngle;
    ...
    odom_l = modulo(odom_l + up_or_down(wp_l_previous, wheel_pos), 9000);
    wp_l_previous = wheel_pos;
 ...
 // motAngleRight = rtY_Right.a_elecAngle;
    ...
    odom_r = modulo(odom_r - up_or_down(wp_r_previous, wheel_pos), 9000);
    wp_r_previous = wheel_pos;
